...
for (m in M3){
  ...
  for (r in RATIO) {
    for (i in 1:1000) {
       ...
       df_rev_log_reg[nrow(df_rev_log_reg) + 1, ] = c(rev_log_reg(x, rcauchy, r*m, alpha, alpha, pm_barre, pn_cauchy), m, r, "cauchy")
       
    }
    for (i in 1:1000) {
    
       df_rev_log_reg[nrow(df_rev_log_reg) + 1, ] = c(rev_log_reg(x, rnorm, r*m, alpha, alpha, pm_barre, pn_norm), m, r, "normale")
       
    }
    for (i in 1:1000) {
    
       df_rev_log_reg[nrow(df_rev_log_reg) + 1, ] = c(rev_log_reg(x, runif, r*m, alpha, psi2, pm_barre, pn_unif), m, r, "unif")
...
pm_barre_ising = function(samples, beta, model = "ising"){
  ...
  for (i in 0:(m-1)){
    ...
    energy_of_each_config = append(energy_of_each_config, compute_energy(samples[id_beginning_config:id_end_config,], model))
  }
...
mc_mle_ising = function(psi, 
  ...
  # Fonction objectif
  L = function(beta){ return(sum(log(pm_barre_ising(x_,beta)/pm_barre_ising(x_,psi))) - m*log(mean(pm_barre_ising(y,beta)/pm_barre_ising(y,psi))))}
...
```{r eval = FALSE}
'
#------- ESSAI DE DIFFERENTTES VALEURS POUR PSI --------------------------------
...
L = function(beta){ return(sum(log(pm_barre_ising(x,beta)/pm_barre_ising(x,psi))) - m*log(mean(pm_barre_ising(y_mcmle,beta)/pm_barre_ising(y_mcmle,psi))))}
...
rli_ising = function (x_ = x, 
  ...
  denom = function(sample, eta) {
    return(pm_barre_ising(sample, beta_, model)*exp(eta[1]) + pn_discr_unif_2D(sample,c(height_, width_))*exp(eta[2]))}
  p_X = function(sample, eta){
