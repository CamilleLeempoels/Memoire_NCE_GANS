...
On définit la fonction \textit{coeur} qui est à une constante de normalisation près la mesure de Gibbs associée à au modèle d'Ising de paramètre \Beta.
On définit la fonction \textit{ising1D(n)} qui est rend toutes les configurations de spins possibles d'un modèle d'Ising unidimensionnel à $n$ sites. 
...
n       = 15   # nombre de sites 
config  = ising(n) # ensemble des configurations 1D à n sites
beta    = 0.9  # paramètre de température
energie = c()

for (k in 1:2**n){ energie = c(energie,totalEnergy1D(config[k,], 1, 0)) } # totalEnergy1D(config[k,], 1, 0), 1 interraction voisins, 0 champs ext
...
```{r}
df_hasting_iid_melted = melt(df_hasting_iid[,-6], id.vars = "epsilon")
```

```{r}
graph1 = ggplot(df_hasting_iid_melted, aes(x = epsilon, y = value)) + geom_line(aes(color = variable, group = variable))

graph2 = ggplot(df_hasting_iid, aes(x = epsilon, y = time)) + geom_line()
...
df_rev_log_reg_param = data.frame(matrix(ncol = 2, nrow = 0))
colnames(df_rev_log_reg_param) = c("const", "ratio_alpha_psi")

alpha = c(2,4)

for (r in 1:4) {
  for (i in 1:50) {
     df_rev_log_reg_param[nrow(df_rev_log_reg_param) + 1, ] = c(rev_log_reg(x, alpha, 10000, alpha*r, pm_barre, 2), r)
  }
}
df_rev_log_reg_param$const_error = abs(df_rev_log_reg_param$const - 4*sqrt(2*pi))
write.csv(df_rev_log_reg_param, "df_rev_log_reg_param.csv")
```

```{r}
df_rev_log_reg_param <- read_csv("df_rev_log_reg_param.csv", show_col_types = FALSE)[,-1]
df_rev_log_reg_param$ratio_alpha_psi = as.factor(df_rev_log_reg_param$ratio_alpha_psi)
ggplot(df_rev_log_reg_param, aes(x=ratio_alpha_psi, y = const_error, group = ratio_alpha_psi, color = ratio_alpha_psi)) + geom_boxplot()
```
```{r eval=FALSE, include=FALSE}
df_rev_log_reg2 = data.frame(matrix(ncol = 4, nrow = 0))
colnames(df_rev_log_reg2) = c("const", "size_data", "ratio_noise_data", "ratio_alpha_psi")
...
for (m in M){
  ...
  for (n in N) {
    for (r in 1:4) {
      for (i in 1:15) {
         df_rev_log_reg2[nrow(df_rev_log_reg2) + 1, ] = c(rev_log_reg(x, alpha, n*m, alpha*r, pm_barre, 2), m, n, r)
      }
    }
  }
}

write.csv(df_rev_log_reg2, "df_rev_log_reg2.csv")
```

```{r}
df_test = aggregate(const ~ size_data + ratio_noise_data + ratio_alpha_psi,
                    data = df_rev_log_reg,
                    FUN = mean)
df_test$ratio_alpha_psi = as.numeric(df_test$ratio_alpha_psi)
df_test$ratio_noise_data = as.factor(df_test$ratio_noise_data)
df_test$size_data = as.factor(df_test$size_data)
df_test$const_error = abs(df_test$const - 4*sqrt(2*pi))
```

```{r}
ggplot(df_test, aes(x = ratio_alpha_psi, y = const_error, color = size_data, shape = ratio_noise_data)) + geom_line() + geom_poinUUt()
```
