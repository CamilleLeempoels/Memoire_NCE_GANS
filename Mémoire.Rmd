...
png('hist.png', units="cm", width=15, height=11, res=300)
...
write.csv(df_recurs_naif, "df_recurs_naif2.csv")
```
...
df_recurs_naif <- read_csv("df_recurs_naif2.csv")[,-1]
colnames(df_recurs_naif) = c("iteration", "param_1", "param_2")
...
ggplot(df_recurs_naif_melted, aes(x = iteration, y = value)) + geom_line(aes(color = variable, group = variable)) + scale_color_manual(values = pal2, labels = lab) + labs(col=" ") + ylab("Valeurs des paramètres") + xlab("Itérations") + geom_hline(yintercept = 2, linetype="dotted") + geom_hline(yintercept = 4, linetype="dotted")
...
write.csv(df_rev_log_reg_noise, "df_rev_log_reg_noise2.csv")
```

```{r message = FALSE}
df_rev_log_reg_noise <- read_csv("df_rev_log_reg_noise2.csv")[,-1]
