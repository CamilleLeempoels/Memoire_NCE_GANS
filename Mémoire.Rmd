...
sim_unif_2D = function(heigth, width){
...
samples_unif_2D = function(n, height, width){
  return(init_ising(n * height, width))
}
...
compute_energy = function(sample, height, width){
  sample = matrix(sample, ncol = width, nrow = height)
  energy = 0
  for (i in 1:height){
    for (j in 1:width){
...
```{r}
samples_ising_2D = function(beta, heigth, width, burn_in, epsilon, m){
  ...
  current_config = sim_ising_2D(beta, heigth, width, burn_in, current_config)
...
```{r}
pm_barre_ising = function(samples, theta){
  # theta[1] = beta
  m = dim(samples)[1] / height
  samples_split = split(samples, as.factor(1:m))
  energie = sapply(samples_split, compute_energy, height, width) 
  return(exp(-theta[1] * energie))
}
...
log_pm_ising = function(samples, theta){
  ...
  samples_split = split(samples, as.factor(1:m))
  energie = sapply(samples_split, compute_energy, height, width) 
  return(-theta[1] * energie + theta[2])
}
...
pn_discr_unif_2D = function(samples, param){
  ...
  return(rowProds(ifelse(samples == -1, 1/2, ifelse(samples == 1, 1, 0))))
}

log_pm_discr_unif_2D = function(samples){
...
psi = 0.8
y = samples_ising_2D(psi, height, width, burn_in, epsilon, m)

# Fonction objectif
L = function(theta){ return(sum(log(pm_barre_ising(x,theta)/pm_barre_ising(x,c(psi)))) - m*log(mean(pm_barre_ising(y,theta)/pm_barre_ising(y,c(psi)))))}

# j'initialise optim avec un paramètre de taille 2 car la dimension 1 ne fonctionne pas avec optim, mais ce n'est pas grave on ne prendra en compte que theta[1]
theta = optim( par = c(1, 1), gr = 'CG', control = list(fnscale=-1), fn = L)$par[1]
```
...
# notre application
rev_log_reg_with_noise(x, samples_unif_2D, n, c(beta), c(height, width), pm_barre_ising, pn_discr_unif_2D)

# résultat du package GiRaf
NC.mrf(height, width, beta)
```
```{r}
# ------- NCE --------------------------------------------------------------------------#

nce(x, samples_unif_2D, c(height, width), log_pm_ising, log_pm_discr_unif_2D, 2, n)
...
\newpage
\newpage
\section{Anciennes versions du code pour Ising}
